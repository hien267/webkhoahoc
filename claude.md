...
- **`index.html`** — Toàn bộ cấu trúc trang: Header, Hero, About, Courses Showcase, Survey (4 panel + stepper), Forum, 2 Modal (Video học thử, Checkout QR), Footer, và `<script>` cấu hình Telegram ở cuối file (trước `</body>`). **Đã gỡ bỏ hoàn toàn section Lịch Học Trực Tuyến / Calendar Widget (`#calendar-section`) theo yêu cầu người dùng (2026-09-22).**
- **`css/style.css`** — CSS Variables cho design tokens khớp mục 2.2. Khu vực responsive nằm cuối file. (Đã gỡ toàn bộ block style Calendar Widget.)
- **`js/data.js`** (~685 dòng) — `BANK_CONFIG`, `COURSES_DATA` (8 khóa học), `FORUM_POSTS_DATA`, `SURVEY_QUESTIONS`, `WEEKDAY_MAP` (không còn dùng sau khi gỡ Calendar Widget, có thể dọn tiếp nếu cần), và `calculateCourseRecommendations()`.
- **`js/app.js`** — `state`, render động câu hỏi khảo sát/grid khóa học/kết quả, xử lý modal, xử lý forum, validate form, toast, confetti, `localStorage`, và **Lead Tracking âm thầm** (`upsertLead`, `loadLeads`, `saveLeads`) — gọi `sendTelegramNotification()` (định nghĩa trong `index.html`) tại 2 thời điểm khảo sát xong / xác nhận thanh toán. (Đã gỡ toàn bộ hàm Calendar Widget: `renderCalendarWidget`, `buildClassDayIndex`, `getCourseScheduleWeekdays`, `jumpCalendarToCourse`, v.v., cùng nút "Xem & Mở Khóa Lịch Học Của Bạn" trong modal thanh toán thành công.)
...
**Việc vừa hoàn thành gần nhất (2026-09-22):** Gỡ bỏ hoàn toàn section Lịch Học Trực Tuyến (Calendar Widget, `#calendar-section`) khỏi giao diện theo yêu cầu người dùng — xoá HTML, CSS và toàn bộ hàm JS liên quan, cùng nút "Xem & Mở Khóa Lịch Học Của Bạn" trong modal thanh toán thành công (nút này chỉ dùng để nhảy tới lịch, nay không còn tác dụng nên gỡ theo). Các phần khác (khảo sát, thanh toán, forum, Telegram webhook, Lead tracking) giữ nguyên không đổi.
